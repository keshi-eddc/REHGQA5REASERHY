...
import com.edmi.site.dianping.http.DianPingCommonDriver;
 ...
 *
 */
public class DianPingShopCommentCrawlByDriver implements Runnable {
	...
	public DianPingShopCommentCrawlByDriver(DianpingShopInfo dianpingShopInfo) {
		super();
	...
	private boolean parseComment(int page, int totalPage) {
		...
		String html = DianPingCommonDriver.getShopComment(header);
		Document doc = Jsoup.parse(html);
		Elements commentList = doc.select("#reviewlist-wrapper ul li .main-review");
...
//		header.setReferer("http://www.dianping.com/shop/" + dianpingShopInfo.getShopId() + "/review_all");
		...
		String html = DianPingCommonRequest.getShopCommentNew(header);
		Document doc = Jsoup.parse(html);
//		log.info(html);
